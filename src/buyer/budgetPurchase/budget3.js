  ...
  addCategory(event) {
    ...
    ListOfBudget[0] &&
      ListOfBudget[0].budgetYearRequests.forEach(function(element, index) {
        ...
        let item = _this.createObject(dateValue,element.checkYear,element.yearlyAmount[0].checkQauter,element.yearlyAmount[0].quarterlyAmount[0].checkMonth);
...
indexMonth,name);
  ...
  handleChangeRequest(e) {
    ...
    //   let listOfRevisions = [];
    //   let listOfForecasts = [];
    //   if(keyIndex != '' && this.state.listOfForecastYears[keyIndex] 
    //     && this.state.listOfForecastYears[keyIndex].listOfRevisions){
    //       listOfRevisions = this.state.listOfForecastYears[keyIndex].listOfRevisions;
    //       listOfForecasts = this.state.listOfForecastYears[keyIndex].listOfForecasts;
    //     }
    //     this.setState({
    //       listOfForecasts: listOfForecasts,
    //       listOfRevisions: listOfRevisions,
  ...
  handlefilterList(event){
    var updatedList = this.state.ListOfBudgetAll;
    updatedList = updatedList.filter(function(item){
      if(item.budgetItemStatus){
      return item.budgetItemStatus.toLowerCase().search(
        event.target.value.toLowerCase()) !== -1;
      }
    });
    if(this.state.ListOfBudgetAll &&  this.state.ListOfBudgetAll.length == 1 && !this.state.ListOfBudgetAll[0].id){

    }
    else
      this.setState({ListOfBudget: updatedList});
  }
...
this.setState({ ListOfBudget: ListOfBudget });
  ...
  render() {
    ...
    let lastMonth1 = '';
    let _this = this;
    console.log('ListOfBudget============', this.state.ListOfBudget, this.state.ListOfBudgetAll,this.state.deleteArray);
    console.log("handleGetExtraData----------------",this.state.listOfRevisions ,'@@@@@@@@@@@@', this.state.listOfForecasts);
    return (
      <div>
      <Header {...this.props} />
      <SideBar
                  ...
                  {this.state.listOfBlueComparision ? (
                    <FormGroup controlId="formControlsSelect">
                      <FormControl
                        ...
                        onChange={event => {
                          this.handleGetExtraData('blueComparision');
                        }}
                      >
                        <option value="select">
                  ...
                  {this.state.listOfRedComparision ? (
                    <FormGroup controlId="formControlsSelect">
                      <FormControl
                        ...
                        name="department"
                        onChange={event => {
                          this.handleGetExtraData('redComparision');
                        }}
                      >
                        <option value="select">
                          Revision X Forecast FY2018-19
                            ...
                            return (
                              ...
                              <td className="w-60"> 
                               ...
                               {elem.sectorTitle?(<span class="tooltiptext">{elem.sectorTitle}</span>):''}
                                  <DropdownButton
                                    title={'Product Line Classification'}
                                    name="Select specification"
                                    className="dropbgwhite w-200 word-break"
                                  >
                                    <ul>
